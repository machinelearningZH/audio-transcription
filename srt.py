...
def create_srt(data):
        ...
        if length < max_length:
            ...
            while word_index < len(segment['words']):
                ...
                while word_index < len(segment['words']):
                    ...
                    if new_segment['start'] == -1 and 'start' in segment['words'][word_index]:
                    ...
                    new_segment['words'].append(copy.deepcopy(segment['words'][word_index]))
                    ...
                    if word_index >= len(segment['words']) or hard_max_length < (current_length + len(segment['words'][word_index]['word'])):
                        break
                    ...
                    if current_length > target_length * 0.5 and (',' in segment['words'][word_index - 1]['word'] or 
                                                                 '«' in segment['words'][word_index]['word'] or 
                                                                 '»' in segment['words'][word_index - 1]['word'] or 
                                                                 'und' in segment['words'][word_index]['word'] or 
                                                                 'oder' in segment['words'][word_index]['word']):
                        break
                    if abs(target_length - current_length) < abs(target_length - (current_length + len(segment['words'][word_index]['word']))):
                        break
    ...
    for i, segment in enumerate(data_srt):
        ...
        if i + 1 < len(data_srt) and (length / 13) < display_time and data_srt[i + 1]['start'] > segment['end']:
            optimal_time_increase = display_time - (length / 13)
            segment['end'] = min(data_srt[i + 1]['start'], segment['end'] + optimal_time_increase)


    text = ''
    for i, segment in enumerate(data_srt):
        text += str(i + 1) + '\n'
        text += "{:0>8}".format(str(datetime.timedelta(seconds=int(segment['start'])))) + ',' + str(int(segment['start'] % 1 * 1000)).ljust(3, '0') + ' --> ' + "{:0>8}".format(str(datetime.timedelta(seconds=int(segment['end'])))) + ',' + str(int(segment['end'] % 1 * 1000)).ljust(3, '0') + '\n'
        ...
        if length > 40:
            ...
            for index in range(len(segment_tokens_lengths)):
                difference = abs(sum(segment_tokens_lengths[index:]) - sum(segment_tokens_lengths[:index]))
                if best_difference > difference:
                    best_difference = difference
                    new_line_position = index
            segment_text = ' '.join(segment_tokens[:new_line_position]) + '\n' + ' '.join(segment_tokens[new_line_position:])
